...
struct cgstatic cfg_static[WNI_CFG_MAX] = {
	...
	{WNI_CFG_SUPPORTED_MCS_SET,
	 CFG_CTL_VALID | CFG_CTL_RE | CFG_CTL_WE | CFG_CTL_SAVE | CFG_CTL_RESTART |
	 CFG_CTL_NTF_LIM,
	 0, 0, 0},
	...
	{WNI_CFG_DEL_ALL_RX_BA_SESSIONS_2_4_G_BTC,
	 CFG_CTL_VALID | CFG_CTL_RE | CFG_CTL_WE | CFG_CTL_INT |
	 CFG_CTL_NTF_LIM,
	 WNI_CFG_DEL_ALL_RX_BA_SESSIONS_2_4_G_BTC_STAMIN,
	 WNI_CFG_DEL_ALL_RX_BA_SESSIONS_2_4_G_BTC_STAMAX,
	 WNI_CFG_DEL_ALL_RX_BA_SESSIONS_2_4_G_BTC_STADEF},
...
struct cfgstatic_string cfg_static_string[CFG_MAX_STATIC_STRING] = {
	...
	{WNI_CFG_VALID_CHANNEL_LIST,
	 ...
	 55,
	{36, 40, 44, 48, 52, 56, 60, 64, 1, 6, 11, 34, 38, 42, 46, 2, 3, 4, 5,
	 7, 8, 9, 10, 12, 13, 14, 100, 104, 108, 112, 116, 120, 124, 128, 132,
	 136, 140, 149, 151, 153, 155, 157, 159, 161, 50, 54, 58, 62, 240, 242,
	 244, 246, 248, 250, 252}},
	...
	{WNI_CFG_EDCA_ANI_ACBE,
	 ...
	 17,
	 {0x0, 0x2, 0x0, 0xf, 0x3, 0xff, 0x64, 0x0, 0x1f, 0x3, 0xff, 0x64,
	  0x0, 0xf, 0x3, 0xff, 0x64}},
	...
	{WNI_CFG_EDCA_WME_ACBE_LOCAL,
	 ...
	 17,
	 {0x0, 0x3, 0x0, 0xf, 0x0, 0x3f, 0x0, 0x0, 0x1f, 0x3, 0xff,0x0, 0x0,
	  0xf, 0x0, 0x3f, 0x0}},

	{WNI_CFG_EDCA_WME_ACVI_LOCAL,
	 WNI_CFG_EDCA_WME_ACVI_LOCAL_LEN,
	 17,
	 {0x0, 0x1, 0x0, 0x7, 0x0, 0xf, 0x5e, 0x0, 0x7, 0x0, 0xf, 0xbc, 0x0,
	  0x7, 0x0, 0xf, 0x5e}},

	{WNI_CFG_EDCA_WME_ACVO_LOCAL,
	 WNI_CFG_EDCA_WME_ACVO_LOCAL_LEN,
	 17,
	 {0x0, 0x1, 0x0, 0x3, 0x0, 0x7, 0x2f, 0x0, 0x3, 0x0, 0x7, 0x66, 0x0,
	  0x3, 0x0, 0x7, 0x2f}},

	{WNI_CFG_EDCA_WME_ACBK,
	 WNI_CFG_EDCA_WME_ACBK_LEN,
	 17,
	 {0x0, 0x7, 0x0, 0xf, 0x3, 0xff, 0x0, 0x0, 0xf, 0x3, 0xff, 0x0, 0x0,
	  0xf, 0x3, 0xff, 0x0}},

	{WNI_CFG_EDCA_WME_ACBE,
	 WNI_CFG_EDCA_WME_ACBE_LEN,
	 17,
	 {0x0, 0x3, 0x0, 0xf, 0x3, 0xff, 0x0, 0x0, 0xf, 0x3, 0xff, 0x0, 0x0,
	  0xf, 0x3, 0xff, 0x0}},

	{WNI_CFG_EDCA_WME_ACVI,
	 WNI_CFG_EDCA_WME_ACVI_LEN,
	 17,
	 {0x0, 0x2, 0x0, 0x7, 0x0, 0xf, 0x5e, 0x0, 0x7, 0x0, 0xf, 0xbc, 0x0,
	  0x7, 0x0, 0xf, 0x5e}},
	...
	{WNI_CFG_RADAR_CHANNEL_LIST,
	 ...
	 15,
	 {0x34, 0x38, 0x3c, 0x40, 0x64, 0x68, 0x6c, 0x70, 0x74, 0x78, 0x7c,
	  0x80, 0x84, 0x88, 0x8c}},
 ...
 *
 * FUNCTION: This function reads the cfg_static array and
...
processCfgDownloadReq(tpAniSirGlobal pMac)
{
    ...
    uint8_t     len;
    struct      cfgstatic_string *str_cfg;
    uint32_t    buf_start, buf_end, log_level, ret_val;
    uint32_t    icount = 0;
    uint32_t    scount = 0;
            ...
            if (!(cfg_static[i].control & CFG_CTL_INT)) {
                ...
                if (str_cfg == NULL) {
                    PELOGE(cfgLog(pMac, LOGE,
                            FL("str_cfg is NULL for ConfigID : %d"),
                            i);)
                    continue;
                }
                index = scount & CFG_BUF_INDX_MASK;
                scount += str_cfg->max_len + 1 + 1;
            } else {
    ...
    for (i = WNI_CFG_MAX - 1; i >= 0; i--)
    {
        ...
        buf_start = pMac->cfg.gCfgEntry[i].control & CFG_BUF_INDX_MASK;
        pMac->cfg.gCfgSBuf[buf_start] = (tANI_U8)(buf_end - buf_start - 2);

        PELOG1(cfgLog(pMac, LOG1, FL("id %d max %d buf_start %d buf_end %d"),
               i, pMac->cfg.gCfgSBuf[buf_start], buf_start, buf_end);)
    ...
    for (i = 0; i < WNI_CFG_MAX ; i++) {
        ...
        if ((pMac->cfg.gCfgEntry[i].control & CFG_CTL_INT) != 0) {
            pMac->cfg.gCfgIBufMin[index] = cfg_static[i].cfg_imin;
            pMac->cfg.gCfgIBufMax[index] = cfg_static[i].cfg_imax;
            pMac->cfg.gCfgIBuf[index]    = cfg_static[i].cfg_ival;
        } else {
            uint8_t max_saved_len;
    ...
    for (i = 0; i < LOG_ENTRY_NUM; i++)
        pMac->utils.gLogEvtLevel[i] = pMac->utils.gLogDbgLevel[i] = log_level;
